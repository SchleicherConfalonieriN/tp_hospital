...
class Turno extends Model {
	...
	public function getTurnosPorUsuario($dni){		
		seguridad::dni_validacion($dni);
		$this->db->query("SELECT turno_id, dni_paciente, servicio, fecha, hora, consultorio, usuarios.nombre, usuarios.apellido from turnos left join usuarios on usuarios.dni=turnos.servicio where dni_paciente = '$dni' AND servicio > 500000 order by fecha , hora");
		return $this->db->fetchAll();
	}
	
	public function getEstudiosPorUsuario($dni){		
		seguridad::dni_validacion($dni);
		$this->db->query("SELECT turno_id, dni_paciente, servicio, fecha, hora, consultorio, estudios.nom_estudio from turnos left join estudios on turnos.servicio=estudios.estudio_id where dni_paciente = '$dni' AND servicio < 500000 order by fecha , hora");
		return $this->db->fetchAll();
	}
